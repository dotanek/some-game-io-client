...
export class EntityFactory {
  public static createNewPlayerEntity(scene: Scene, x: number, y: number): Entity {
    return new Entity(scene, x, y, config.entity.minMass).setOrigin(0.5, 0.5).setFillStyle(0xffffff, 1).setVisible(true);
  }

  public static createOtherPlayerEntity(
    scene: Scene,x: number, y: number, mass: number): Entity {
